...
public class DigitFlip extends View implements Runnable {
  ...
  public DigitFlip(Context context) {
    this(context, null);
  }

  public DigitFlip(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public DigitFlip(Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);
    init(context, attrs);
  }

  @TargetApi(Build.VERSION_CODES.LOLLIPOP)
  public DigitFlip(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
  ...
  public void init(Context context, AttributeSet attrs) {
    ...
    TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.HorizontalWheelView, 0, 0);
    final int num = ta.getIndexCount();
    for (int i = 0; i < num; i++) {
      int attr = ta.getIndex(i);
      if (attr == R.styleable.HorizontalWheelView_textSize) {
        textSize = ta.getDimensionPixelSize(attr, -1);
      } else if (attr == R.styleable.HorizontalWheelView_padding) {
        padding = ta.getDimensionPixelSize(attr, -1);
      } else if (attr == R.styleable.HorizontalWheelView_cornerSizer) {
        cornerSize = ta.getDimensionPixelSize(attr, -1);
      } else if (attr == R.styleable.HorizontalWheelView_textColor) {
        textColor = ta.getColor(attr, 1);
      } else if (attr == R.styleable.HorizontalWheelView_backgroundColor) {
        backgroundColor = ta.getColor(attr, 1);
      } else if (attr == R.styleable.HorizontalWheelView_reverseRotation) {
